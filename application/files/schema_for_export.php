...
********************************************************************************************/
$phpMySQLAutoBackup_version="1.5.2";
// ---------------------------------------------------------
...
$query="SELECT * from phpmysqlautobackup WHERE 1 LIMIT 1 ;";
$result=mysql_query($query);
...
if (time() < (@$row['time_last_run']+$time_internal)) { $already=1; }// exit if already run within last time_interval
...
$query="UPDATE phpmysqlautobackup SET time_last_run = '".time()."' WHERE 1 LIMIT 1 ;";
$result=mysql_query($query);
...
mysql_close();
$buffer = ereg_replace('#phpmysqlautobackup-endtime#', strftime("%H:%M:%S",time()), $buffer);
}
?>
